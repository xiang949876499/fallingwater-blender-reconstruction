...
Q=R/'qa';TAG='main-terrace11-attempt02'
...
def prop_state(obj):
    ...
    for p in obj.bl_rna.properties:
        if p.identifier in ('rna_type','name','users'):continue
        try:
...
def materials():
    ...
    return out
before_fp=fingerprint();before_mats=materials();before_users={m.name:m.users for m in bpy.data.materials};print('Fingerprint ready',flush=True)
