...
def build(ctx):
    ...
    def rounded(w,d,r):
        ...
        return pts
    pooltop=gz+.70485;waterz=pooltop-.14;basin=gz-.91;inner=rounded(w,dep,radius);outer=rounded(w+.29,dep+.29,radius+.145)
    ...
    for i in range(4):
        box('GUEST_POOL_underwater_step_%d'%i,(px-w/2+.32+i*.20,py+dep/2-.58,waterz-.18-i*.19),(.41,1.06,.18),concrete,col,.012)
    stair('GUEST_POOL_coping_ascent',(702,443),(723,443),gz,pooltop,1.0,4)
    planter=[p(q) for q in [[697,446],[733,446],[733,470],[697,470]]]
