...
for n in ['FW_FURN_MAIN_L2_MASTER_walnut_bed_00_recessed_support','FW_FURN_MAIN_L2_MASTER_bedroom_desk_02_leg','FW_FURN_MAIN_L2_MASTER_desk_chair_03_leg','MASTER_DETAIL10_wardrobe_recessed_support','MASTER_DETAIL10_north_bedside_side_-1','MASTER_DETAIL10_south_open_bedside_side_-1']:
 ...
 raw,p,no,f,o,mat=s.ray_cast(deps,Vector((*xy,md.TOP+.05)),Vector((0,0,-1)),distance=.10)
 # A probe inside a shelf can first hit its underside. Test the physical
 # finish separately, retaining that first-hit evidence, instead of calling
 # an intervening furniture shelf a missing floor.
 floor_tree,_=geom(s.objects['MAIN_L2_MASTER_finish'])
 fp,fn,ff,fd=floor_tree.ray_cast(Vector((*xy,md.TOP+.05)),Vector((0,0,-1)),.1)
 support.append({'object':n,'bounds':b,'first_scene_hit':o.name if raw else None,'floor':'MAIN_L2_MASTER_finish' if fp is not None else None,'floor_z':fp.z if fp is not None else None,'base_to_finish':b[2][0]-md.TOP,'pass':fp is not None and abs(fp.z-md.TOP)<.0001 and -.03<=b[2][0]-md.TOP<=.00015})
