...
class Batch:
    ...
    def add(self,ring,z0,z1,rng,description):
        ...
        for t in tessellate_polygon([flat]):
            ids=[min(range(n),key=lambda i:(flat[i]-q).length_squared) for q in t]
            if any((flat[i]-q).length>1e-6 for i,q in zip(ids,t)):raise ValueError('Tessellation created an unexpected vertex')
            a,b,c=ids
