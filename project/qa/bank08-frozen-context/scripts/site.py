...
def _geology(ctx, cfg, rng, coll):
    ...
    for k in range(105):
        ...
        count+=1
    if cfg.get('core_bedrock_revision') in ('discontinuous_beds_v1','discontinuous_beds_v2','discontinuous_beds_v3'):
        result = refine_core_bedrock(cfg)
        count += result['object_count_delta']
    return count


def refine_core_bedrock(cfg):
    """Bounded C-level rock revision, preserving the complete original upper cap.

    This is opt-in pending visual acceptance. Replaces only the original core
    data and eight disconnected cascade shoulder meshes. No river, terrain,
    route, foundation or water geometry is regenerated. Old fluid caches are
    invalid for the changed collision surfaces, even with an unchanged inlet.
    """
    if cfg.get('core_bedrock_revision') == 'discontinuous_beds_v3':
        return refine_core_bedrock_v3(cfg)
    if cfg.get('core_bedrock_revision') == 'discontinuous_beds_v2':
        return refine_core_bedrock_v2(cfg)
    import bmesh

    core = bpy.data.objects['SITE_Core_Continuous_Fractured_Sandstone']
    if core.get('bedrock_revision') == 'discontinuous_beds_v1':
        return {'object_count_delta': 0, 'already_applied': True}

    def install(obj, vertices, faces, top_face_count=1):
        previous = obj.data
        mesh = bpy.data.meshes.new(obj.name + '_DiscontinuousBeds')
        ...
        for polygon in mesh.polygons:
            # Flat broad geological facets; small bevel-like rings are absent.
            polygon.use_smooth = False
        obj.data = mesh
        obj['bedrock_revision'] = 'discontinuous_beds_v1'
        obj['evidence'] = 'C; HABS south elevation 07 and southwest photograph 87 informed morphology, not a site scan'
        obj['old_fluid_cache_status'] = 'INVALID_FOR_CHANGED_COLLISION_MESH'
        return previous
    ...
    old = [tuple(v.co) for v in core.data.vertices]
    perimeter = 53
    if len(old) != perimeter*27:
        raise ValueError('Expected untouched iteration06 core topology (27 x 53)')
    top, bottom = old[:perimeter], old[-perimeter:]
    lip = Vector(cfg['river_path'][8][:2])
    down = (Vector(cfg['river_path'][9][:2])-lip).normalized()
    across = Vector((-down.y, down.x))
    vertices = list(top); faces = []
    # Broad, oblique fracture planes are defined independently of minor bedding.
    # Five uneven major profile changes replace the former nine identical bellies.
    rows = 25
    for row in range(1, rows+1):
        t = row/rows
        for j, (upper, lower) in enumerate(zip(top, bottom)):
            if row == rows:
                vertices.append(lower); continue
            z = upper[2] + (lower[2]-upper[2])*t
            if j >= 49:
                vertices.append((upper[0]+(lower[0]-upper[0])*t,
                                 upper[1]+(lower[1]-upper[1])*t, z))
                continue
            c = -6.2+j*12.4/48
            old_top_s = (Vector(upper[:2])-lip).dot(down)
            end_s = (Vector(lower[:2])-lip).dot(down)
            s = old_top_s + (end_s-old_top_s)*t
            shoulder_profile = linear(t, [(0,0),(.18,-.035),(.39,.045),
                                          (.63,-.105),(.84,-.015),(1,0)])
            # Unequal panel depth offsets continue across several strata. No
            # wholesale sinusoidal swelling of the rock body or random boulders.
            panel = -.032 if c < -3.6 else (.024 if c < .43 else (-.015 if c < 3.15 else .018))
            envelope = min(1, t/.085, (1-t)/.085)
            joint = sum(-.125*math.exp(-((c-q)/.105)**2) for q in (-3.6,.43,3.15))
            seams = 0
            for elevation, lo, hi in [(-3.76,-6.2,-.9),(-4.49,-2.3,4.6),(-5.58,.6,6.2)]:
                if lo < c < hi:
                    lateral = min(1,(c-lo)/.40,(hi-c)/.40)
                    seams -= .024*lateral*math.exp(-((z-elevation-c*.018)/.033)**2)
            s += envelope*(shoulder_profile+panel+joint+seams)
            # Small discontinuities in bedding height; not repeated ring bulges.
            z += envelope*(.012*(c/6.2) + (.016 if c>.43 else -.008))
            p = lip + across*c + down*s
            vertices.append((p.x,p.y,z))
    for row in range(rows):
        for j in range(perimeter):
            q = row*perimeter+j; after = row*perimeter+(j+1)%perimeter
            faces.append((q,after,after+perimeter,q+perimeter))
    faces.extend([tuple(reversed(range(perimeter))),
                  tuple(rows*perimeter+j for j in range(perimeter))])
    old_core_mesh = install(core,vertices,faces)
    for polygon in core.data.polygons:
        if len(core.data.materials)>1:
            polygon.material_index = int(abs((polygon.center.x-lip.x)*across.x+
                                              (polygon.center.y-lip.y)*across.y)>4)
    core['upper_cap_preserved_vertex_count'] = perimeter
    core['role'] = 'continuous bedrock: unequal oblique planes, discontinuous shallow beds, cross-bed joints'

    created = []
    for side in (0,1):
        sources = [bpy.data.objects[f'SITE_Cascade_Shoulder_{side}_{j}'] for j in range(4)]
        high, low = sources[0], sources[-1]
        high_n = (len(high.data.vertices)-1)//3
        low_n = (len(low.data.vertices)-1)//3
        top_ring = [tuple(v.co) for v in high.data.vertices[2*high_n:3*high_n]]
        top_center = tuple(high.data.vertices[-1].co)
        low_ring = [tuple(v.co) for v in low.data.vertices[:low_n]]
        cx = sum(p[0] for p in low_ring)/low_n; cy = sum(p[1] for p in low_ring)/low_n
        tx = sum(p[0] for p in top_ring)/high_n; ty = sum(p[1] for p in top_ring)/high_n
        polar = sorted((math.atan2(p[1]-cy,p[0]-cx),math.hypot(p[0]-cx,p[1]-cy),p[2]) for p in low_ring)
        polar = [(polar[-1][0]-TAU,*polar[-1][1:])] + polar + [(polar[0][0]+TAU,*polar[0][1:])]
        targets = []
        for p in top_ring:
            a = math.atan2(p[1]-ty,p[0]-tx)
            for left,right in zip(polar,polar[1:]):
                if left[0] <= a <= right[0]:
                    u = (a-left[0])/(right[0]-left[0])
                    radius = left[1]+(right[1]-left[1])*u
                    z = left[2]+(right[2]-left[2])*u
                    targets.append((cx+radius*math.cos(a),cy+radius*math.sin(a),z));break
        if len(targets) != high_n:raise ValueError('Shoulder angular registration failed')
        vv = list(top_ring); ff = []
        levels = (0,.17,.36,.59,.79,1)
        for t in levels[1:]:
            for j,(p,b) in enumerate(zip(top_ring,targets)):
                x=p[0]+(b[0]-p[0])*t;y=p[1]+(b[1]-p[1])*t;z=p[2]+(b[2]-p[2])*t
                # A few vertical joints continue across the whole rock wall;
                # corner-specific recesses do not create stacked convex disks.
                joint = (.055 if j in (2,7) else .008)*math.sin(math.pi*t)
                x += (tx-x)*joint; y += (ty-y)*joint
                vv.append((x,y,z))
        for row in range(len(levels)-1):
            for j in range(high_n):
                q=row*high_n+j;a=row*high_n+(j+1)%high_n
                ff.append((q,a,a+high_n,q+high_n))
        center=len(vv);vv.append(top_center)
        ff.extend((center,j,(j+1)%high_n) for j in range(high_n))
        ff.append(tuple((len(levels)-1)*high_n+j for j in reversed(range(high_n))))
        name=f'SITE_Cascade_Shoulder_Continuous_{side}'
        new=high.copy();new.name=name;high.users_collection[0].objects.link(new)
        install(new,vv,ff,high_n)
        new['upper_cap_preserved_vertex_count']=high_n+1
        new['role']='continuous sandstone shoulder with angular cross-bed fractures; no disconnected four-slab stack'
        created.append(name)
        for source in sources:bpy.data.objects.remove(source,do_unlink=True)
    return {'revision':'discontinuous_beds_v1','object_count_delta':-6,
            'revised_objects':[core.name]+created,'upper_core_cap_vertices_unchanged':perimeter,
            'fluid_cache_compatibility':'OLD_CACHES_INVALID; inlet/upstream cap retained for comparable new tests'}


def refine_core_bedrock_v2(cfg):
    """Four unequal major beds traced from photo87, not full-height fluting.

    Horizontal ledges are explicit mesh rows. The three traced relative depths
    (0.20, 0.46, 0.82) are C estimates: photo perspective and water occlusion
    preclude treating them as measured Fallingwater stratigraphy.
    """
    import bmesh
    core=bpy.data.objects['SITE_Core_Continuous_Fractured_Sandstone']
    if core.get('bedrock_revision') == 'discontinuous_beds_v2':
        return {'object_count_delta':0,'already_applied':True}
    if len(core.data.vertices)!=53*27:
        raise ValueError('07b requires original iteration06 geometry, not the rejected 07 candidate')
    top=[tuple(v.co) for v in core.data.vertices[:53]]
    bottom=[tuple(v.co) for v in core.data.vertices[-53:]]
    original_cfg=dict(cfg);original_cfg['core_bedrock_revision']='discontinuous_beds_v1'
    result=refine_core_bedrock(original_cfg)
    ...
    def replace(obj,vertices,faces):
        previous=obj.data;mesh=bpy.data.meshes.new(obj.name+'_TracedBeds07b')
        mesh.from_pydata(vertices,[],faces);mesh.update()
        for m in previous.materials:mesh.materials.append(m)
        bm=bmesh.new();bm.from_mesh(mesh)
        bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces))
        if bm.calc_volume(signed=True)<0:bmesh.ops.reverse_faces(bm,faces=list(bm.faces))
        bm.to_mesh(mesh);bm.free();mesh.update()
        obj.data=mesh
        obj['bedrock_revision']='discontinuous_beds_v2'
        obj['reference_annotation']='qa/core-geology-07b-reference-annotations.png'
        obj['old_fluid_cache_status']='INVALID_FOR_CHANGED_COLLISION_MESH'

    lip=Vector(cfg['river_path'][8][:2]);down=(Vector(cfg['river_path'][9][:2])-lip).normalized()
    across=Vector((-down.y,down.x))
    # Explicit rows on both sides of every principal ledge. The broad upper
    # cap, receded pocket, lower mass and foot have unequal heights/depths.
    levels=(0,.025,.095,.175,.20,.225,.29,.36,.425,.46,.487,.55,.64,.735,.795,.82,.845,.91,.965,1)
    profile=((0,0),(.025,-.005),(.095,.012),(.175,.075),(.20,.065),
             (.225,-.235),(.29,-.290),(.36,-.255),(.425,-.135),
             (.46,.125),(.487,.095),(.55,-.018),(.64,-.032),
             (.735,.002),(.795,.068),(.82,.045),(.845,-.135),
             (.91,-.098),(.965,-.025),(1,0))
    vertices=[];faces=[]
    for row,t in enumerate(levels):
        for j,(up,lo) in enumerate(zip(top,bottom)):
            if row==0:vertices.append(up);continue
            if row==len(levels)-1:vertices.append(lo);continue
            if j>=49:
                vertices.append(tuple(up[k]+(lo[k]-up[k])*t for k in range(3)));continue
            c=-6.2+j*12.4/48
            # A few broad, slightly oblique rock faces; do not extrude the lip's
            # small sinusoidal wave pattern into columns down the entire face.
            planar=interp(c,[(-6.2,-.17),(-3.3,-.11),(-.8,-.225),(2.65,-.15),(6.2,-.24)])
            base=planar-.18*t
            relief=interp(t,profile)
            strength=interp(c,[(-6.2,.85),(-3.7,1.00),(-1.3,.72),(1.35,.94),(3.9,.58),(6.2,.88)])
            # Different lateral extents of two principal seams make them fade
            # into the same body, not independent pancakes spanning all width.
            if .20<=t<=.425:
                strength*=interp(c,[(-6.2,1),(-2.8,.85),(.3,.35),(2.1,.83),(6.2,1)])
            if .795<=t<=.91:
                strength*=interp(c,[(-6.2,.40),(-1.8,.38),(1.1,1),(6.2,.88)])
            s=base+relief*strength
            # Two localized faults die out at bedding boundaries. No groove
            # runs from the upper cap to the pool, and no repeated panel offsets.
            for center,start,end,depth in [(-2.4,.11,.47,.072),(3.35,.47,.86,.060)]:
                if start<t<end:
                    fade=min(1,(t-start)/.07,(end-t)/.07)
                    s-=depth*max(0,1-abs(c-center)/.19)*fade
            # Preserve original top/base boundary exactly, but contain their
            # high-frequency contour within short transitions near those caps.
            top_s=(Vector(up[:2])-lip).dot(down);bottom_s=(Vector(lo[:2])-lip).dot(down)
            if t<.095:
                a=t/.095;s=top_s*(1-a)+s*a
            if t>.965:
                a=(t-.965)/.035;s=s*(1-a)+bottom_s*a
            shift=interp(c,[(-6.2,.010),(-3.2,-.008),(-.6,.018),(2.3,-.012),(6.2,.005)])
            tt=t+shift*min(1,t/.12,(1-t)/.10)
            z=up[2]+(lo[2]-up[2])*tt
            p=lip+across*c+down*s
            vertices.append((p.x,p.y,z))
    n=53
    for row in range(len(levels)-1):
        for j in range(n):
            q=row*n+j;a=row*n+(j+1)%n
            faces.append((q,a,a+n,q+n))
    faces.extend([tuple(reversed(range(n))),tuple((len(levels)-1)*n+j for j in range(n))])
    replace(core,vertices,faces)
    for polygon in core.data.polygons:
        polygon.material_index=int(abs((polygon.center.x-lip.x)*across.x+
                                        (polygon.center.y-lip.y)*across.y)>4)
    core['profile_normalized_principal_depths']=[.20,.46,.82]
    core['role']='four unequal continuous sandstone beds: thick cap, receded pocket, interrupted middle ledge, lower foot'
    core['lower_cap_start_vertex']=(len(levels)-1)*n

    for side in (0,1):
        obj=bpy.data.objects[f'SITE_Cascade_Shoulder_Continuous_{side}']
        n=(len(obj.data.vertices)-1)//6
        up=[tuple(v.co) for v in obj.data.vertices[:n]]
        lo=[tuple(v.co) for v in obj.data.vertices[5*n:6*n]]
        center=tuple(obj.data.vertices[-1].co)
        tx=sum(p[0] for p in up)/n;ty=sum(p[1] for p in up)/n
        bx=sum(p[0] for p in lo)/n;by=sum(p[1] for p in lo)/n
        upper_z=sum(p[2] for p in up)/n;lower_z=sum(p[2] for p in lo)/n
        # Bed boundaries align approximately with the core in world Z. Different
        # shoulder heights therefore expose different numbers of major beds.
        heights=[-3.39,-3.60,-3.70,-3.80,-3.96,-4.20,-4.43,-4.56,-4.65,-4.80,-5.04,-5.28]
        ss=[0]+sorted((upper_z-z)/(upper_z-lower_z) for z in heights if lower_z+.035<z<upper_z-.025)+[1]
        vv=[];ff=[]
        for t in ss:
            for j,(p,b) in enumerate(zip(up,lo)):
                if t==0:vv.append(p);continue
                if t==1:vv.append(b);continue
                x=p[0]+(b[0]-p[0])*t;y=p[1]+(b[1]-p[1])*t;z=p[2]+(b[2]-p[2])*t
                average_z=upper_z+(lower_z-upper_z)*t
                relief=interp(-average_z,[(3.20,0),(3.39,.012),(3.60,.052),(3.70,.038),
                           (3.80,-.180),(3.96,-.205),(4.20,-.132),(4.43,.068),
                           (4.56,.052),(4.65,-.120),(4.80,-.095),(5.04,-.020),(5.45,0)])
                # Some bed edges end at existing angular fractures rather than
                # extending with equal strength around the entire perimeter.
                strength=(.46 if j in (0,1,2) else (.73 if j in (6,7) else 1.0))
                if side==1:strength=.65+.35*strength
                ccx=tx+(bx-tx)*t;ccy=ty+(by-ty)*t
                radius=max(1e-5,math.hypot(x-ccx,y-ccy));offset=relief*strength
                x+=(x-ccx)/radius*offset;y+=(y-ccy)/radius*offset
                vv.append((x,y,z))
        for row in range(len(ss)-1):
            for j in range(n):
                q=row*n+j;a=row*n+(j+1)%n;ff.append((q,a,a+n,q+n))
        ci=len(vv);vv.append(center)
        ff.extend((ci,j,(j+1)%n) for j in range(n))
        ff.append(tuple((len(ss)-1)*n+j for j in reversed(range(n))))
        replace(obj,vv,ff)
        obj['role']='continuous shoulder: unequal partial horizontal ledges and angular massive beds'
    result['revision']='discontinuous_beds_v2'
    result['core_lower_cap_start_vertex']=(len(levels)-1)*53
    result['reference_annotation']='qa/core-geology-07b-reference-annotations.png'
    return result


def refine_core_bedrock_v3(cfg):
    """Localized mesoscale spalling on v2's source-informed major beds.

    These C-level erosion polygons are authored examples of fracture types
    visible in photo87, not photogrammetrically located cuts. Keep the exact
    hydraulic caps, materials and all non-rock objects; do not rescale texture.
    """
    import bmesh
    core=bpy.data.objects['SITE_Core_Continuous_Fractured_Sandstone']
    if core.get('bedrock_revision') == 'discontinuous_beds_v3':
        return {'object_count_delta':0,'already_applied':True}
    source_cfg=dict(cfg);source_cfg['core_bedrock_revision']='discontinuous_beds_v2'
    result=refine_core_bedrock_v2(source_cfg)
    lip=Vector(cfg['river_path'][8][:2]);down=(Vector(cfg['river_path'][9][:2])-lip).normalized()
    across=Vector((-down.y,down.x))
    # c is distance across the existing front, d is depth below z=-3.055.
    # Each patch is a distinct partial spall, not one repeated cross-section.
    patches=[
      ('upper_left_spall',[(-5.70,.18),(-4.65,.27),(-4.12,.57),(-4.62,.71),(-5.48,.56)],.072),
      ('left_recess_fracture',[(-4.04,.86),(-3.24,.72),(-2.83,1.07),(-3.11,1.36),(-3.80,1.29)],.095),
      ('lower_left_split',[(-5.29,1.80),(-4.46,1.86),(-4.16,2.44),(-4.65,2.60),(-5.33,2.29)],.063),
      ('central_lower_spall',[(-2.26,1.91),(-1.46,1.77),(-.87,2.13),(-1.00,2.65),(-1.73,2.77),(-2.35,2.45)],.116),
      ('upper_center_shear',[(.03,.24),(.90,.28),(1.34,.50),(.92,.83),(.21,.72)],.069),
      ('middle_right_break',[(1.76,1.14),(2.34,1.04),(2.89,1.36),(2.69,1.78),(1.97,1.67)],.102),
      ('right_lower_spall',[(3.57,2.01),(4.42,2.12),(4.91,2.71),(4.56,3.00),(3.67,2.82),(3.35,2.38)],.078),
      ('right_upper_corner',[(4.31,.25),(5.51,.34),(5.78,.69),(5.23,1.04),(4.51,.82)],.088)]
    cracks=[
      ([(-5.78,.60),(-4.96,.64),(-4.18,.56),(-3.62,.74)],.062,.066),
      ([(-2.21,1.37),(-1.26,1.43),(-.36,1.30),(.56,1.36),(1.24,1.26)],.052,.079),
      ([(2.82,2.43),(3.41,2.49),(3.94,2.65),(4.76,2.60)],.068,.072),
      ([(1.17,.17),(.99,.38),(1.16,.68),(1.00,.94)],.049,.062),
      ([(-3.06,1.86),(-2.81,2.12),(-2.92,2.48),(-2.72,2.72)],.072,.092)]

    def noise2(x,y,salt=0):
        ix=math.floor(x);iy=math.floor(y);fx=x-ix;fy=y-iy
        fx=fx*fx*(3-2*fx);fy=fy*fy*(3-2*fy)
        def value(a,b):
            n=(a*374761393+b*668265263+salt*1274126177)&0xffffffff
            n=((n^(n>>13))*1274126177)&0xffffffff;n^=n>>16
    ...
    def polygon_distance(x,y,polygon):
        inside=False;distance=1e20
        for a,b in zip(polygon,polygon[1:]+polygon[:1]):
            distance=min(distance,_segment_near(x,y,a,b)[0])
            if (a[1]>y)!=(b[1]>y) and x<(b[0]-a[0])*(y-a[1])/(b[1]-a[1])+a[0]:inside=not inside
        return distance if inside else -distance

    def relief(c,d):
        h=.026*noise2(c*3.7,d*4.1,7)+.009*noise2(c*10.4,d*9.2,19)
        dz=.012*noise2(c*2.3,d*3.9,31)
        for name,polygon,depth in patches:
            dist=polygon_distance(c,d,polygon)
            if dist>0:
                weight=min(1,dist/.075)
                cx=sum(p[0] for p in polygon)/len(polygon);dy=sum(p[1] for p in polygon)/len(polygon)
                h-=weight*(depth+.018*(c-cx)-.012*(d-dy))
                dz+=weight*.013*noise2(c*5.1,d*3.8,47)
        for points,width,depth in cracks:
            dist=min(_segment_near(c,d,a,b)[0] for a,b in zip(points,points[1:]))
            if dist<width:h-=depth*(1-dist/width)**.7
        # Small edge chips vary within, rather than replacing, the major beds.
        close=min(abs(d-z) for z in (.67,1.54,2.75))
        if close<.13:dz+=.023*(1-close/.13)*noise2(c*3.5,d*1.3,63)
        return h,dz

    def interpolate_ring(rows,p,j,subdivision):
        a=min(len(rows)-2,int(p));v=p-a
        q=j/subdivision;k=min(len(rows[0])-2,int(q));u=q-k
        upper=Vector(rows[a][k]).lerp(Vector(rows[a][k+1]),u)
        lower=Vector(rows[a+1][k]).lerp(Vector(rows[a+1][k+1]),u)
        return upper.lerp(lower,v)

    def row_samples(rows,max_step):
        samples=[]
        for r in range(len(rows)-1):
            longest=max((Vector(a)-Vector(b)).length for a,b in zip(rows[r],rows[r+1]))
            steps=max(1,math.ceil(longest/max_step))
            for j in range(1,steps+1):
                p=r+j/steps
                if p<len(rows)-1:samples.append(p)
        return samples

    def bridge_rings(faces,coarse_start,dense_start,n,subdivision,dense_n,coarse_above=True):
        # Original cap edges are never subdivided. Transition n-gons connect
        # them to the dense wall; the hydraulic cap itself stays identical.
        def dense_index(j):
            return j*subdivision if j<=48 and n==53 else (144+j-48 if n==53 else j*subdivision)
        for j in range(n):
            nxt=(j+1)%n;first=dense_index(j);last=dense_index(nxt)
            chain=[first]
            while chain[-1]!=last:chain.append((chain[-1]+1)%dense_n)
            f=[coarse_start+j,coarse_start+nxt]+[dense_start+k for k in reversed(chain)]
            faces.append(tuple(f if coarse_above else reversed(f)))

    def install(obj,vertices,faces,top_count,bottom_start,mat_from_c=False):
        prior=obj.data;mesh=bpy.data.meshes.new(obj.name+'_MesoscaleFractures07c')
        mesh.from_pydata(vertices,[],faces);mesh.update()
        for m in prior.materials:mesh.materials.append(m)
        bm=bmesh.new();bm.from_mesh(mesh);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces))
        if bm.calc_volume(signed=True)<0:bmesh.ops.reverse_faces(bm,faces=list(bm.faces))
        for face in bm.faces:face.smooth=True
        for edge in bm.edges:
            if edge.is_manifold:edge.smooth=edge.calc_face_angle()<math.radians(38)
        bm.to_mesh(mesh);bm.free();mesh.update()
        for p in mesh.polygons:
            if all(v<top_count for v in p.vertices) or all(v>=bottom_start for v in p.vertices):p.use_smooth=False
            if mat_from_c:p.material_index=int(abs((p.center.x-lip.x)*across.x+(p.center.y-lip.y)*across.y)>4)
        obj.data=mesh;obj['bedrock_revision']='discontinuous_beds_v3'
        obj['old_fluid_cache_status']='INVALID_FOR_CHANGED_COLLISION_MESH'
        obj['mesoscale_evidence']='C explicit partial spalls and short cracks informed by photo87; not measured fracture coordinates'
        obj['geometry_roughness_scale_m']=[.06,.35]

    actual_offsets=[]
    old=[tuple(v.co) for v in core.data.vertices]
    rows=[old[i:i+53] for i in range(0,len(old),53)]
    front_rows=[r[:49] for r in rows]
    parameters=row_samples(front_rows,.065)
    vv=list(rows[0]);ff=[];dense_n=149
    for p in parameters:
        r=min(len(rows)-2,int(p));u=p-r
        for j in range(145):
            point=interpolate_ring(front_rows,p,j,3)
            c=(Vector(point[:2])-lip).dot(across);d=-3.055-point.z
            h,dz=relief(c,d)
            # Preserve full fixed top/bottom and the side/back boundary while
            # roughening the face below; no upstream bed deformation.
            fade=min(1,max(0,d/.13),max(0,(point.z+6.28)/.16),max(0,(6.2-abs(c))/.22))
            point.x+=down.x*h*fade;point.y+=down.y*h*fade;point.z+=dz*fade
            actual_offsets.append((abs(h*fade),abs(dz*fade)))
            vv.append(tuple(point))
        for j in range(49,53):vv.append(tuple(Vector(rows[r][j]).lerp(Vector(rows[r+1][j]),u)))
    bottom_start=len(vv);vv.extend(rows[-1])
    bridge_rings(ff,0,53,53,3,dense_n,True)
    for r in range(len(parameters)-1):
        start=53+r*dense_n
        for j in range(dense_n):
            a=start+j;b=start+(j+1)%dense_n;ff.append((a,b,b+dense_n,a+dense_n))
    bridge_rings(ff,bottom_start,53+(len(parameters)-1)*dense_n,53,3,dense_n,False)
    ff.extend([tuple(reversed(range(53))),tuple(bottom_start+j for j in range(53))])
    install(core,vv,ff,53,bottom_start,True)
    core['lower_cap_start_vertex']=bottom_start
    core['role']='four major sandstone beds with local spalling, interrupted short cracks and eroded ledge edges'

    for side in (0,1):
        obj=bpy.data.objects[f'SITE_Cascade_Shoulder_Continuous_{side}']
        n=int(obj['upper_cap_preserved_vertex_count'])-1
        old=[tuple(v.co) for v in obj.data.vertices];center=old[-1]
        rows=[old[i:i+n] for i in range(0,len(old)-1,n)]
        params=row_samples(rows,.065);sub=12;dn=n*sub
        wrapped=[r+[r[0]] for r in rows]
        vv=list(rows[0]);ff=[]
        for p in params:
            r=min(len(rows)-2,int(p));u=p-r
            cp=[Vector(a).lerp(Vector(b),u) for a,b in zip(rows[r],rows[r+1])]
            cc=sum(cp,Vector())/len(cp)
            for j in range(dn):
                point=interpolate_ring(wrapped,p,j,sub)
                radial=Vector((point.x-cc.x,point.y-cc.y,0)).normalized()
                c=(Vector(point[:2])-lip).dot(across);d=-3.055-point.z
                h,dz=relief(c,d)
                front=max(.22,radial.dot(Vector((down.x,down.y,0))))
                fraction=p/(len(rows)-1);fade=min(1,fraction/.075,(1-fraction)/.075)
                point+=radial*(h*front*fade);point.z+=dz*fade
                actual_offsets.append((abs(h*front*fade),abs(dz*fade)))
                vv.append(tuple(point))
        bs=len(vv);vv.extend(rows[-1]);ci=len(vv);vv.append(center)
        bridge_rings(ff,0,n,n,sub,dn,True)
        for r in range(len(params)-1):
            start=n+r*dn
            for j in range(dn):
                a=start+j;b=start+(j+1)%dn;ff.append((a,b,b+dn,a+dn))
        bridge_rings(ff,bs,n+(len(params)-1)*dn,n,sub,dn,False)
        ff.extend((ci,j,(j+1)%n) for j in range(n));ff.append(tuple(bs+j for j in reversed(range(n))))
        install(obj,vv,ff,n,bs,False)
        # The center vertex is the unchanged upper cap's original fan center.
        for poly in obj.data.polygons:
            if ci in poly.vertices:poly.use_smooth=False
        obj['role']='continuous sandstone shoulder with localized mesoscale fractures, not clean triangulated block faces'
    result['revision']='discontinuous_beds_v3'
    result['core_lower_cap_start_vertex']=bottom_start
    result['mesoscale_patches']=[{'id':name,'polygon_c_depth_m':poly,'recession_m':depth} for name,poly,depth in patches]
    result['short_crack_count']=len(cracks)
    result['actual_surface_offsets_m']={'vertices':len(actual_offsets),
             'normal_max':max(p[0] for p in actual_offsets),'vertical_max':max(p[1] for p in actual_offsets),
             'normal_over_1cm_vertices':sum(p[0]>.01 for p in actual_offsets)}
    return result
