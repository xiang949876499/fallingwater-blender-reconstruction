...
class Batch:
    ...
    def add(self,ring,z0,z1,rng,description):
        ...
        for t in tessellate_polygon([flat]):
            # Blender5.2.1 returns point indices, verified against local API.
            if not all(isinstance(i,int) and 0<=i<n for i in t):raise ValueError('Unexpected tessellate_polygon API')
            a,b,c=t
...
def build(ctx):
    ...
    if existing:
        ...
        guards=json.loads(existing.get('source_guards','{}'))
        ...
        if set(guards)!=set(GUARDS) or any(n not in scene.objects or geometry_hash(scene.objects[n])!=h for n,h in guards.items()):raise ValueError('Tower12 source guard changed')
    ...
    if any([m.name if m else None for m in scene.objects[n].data.materials]!=['FW_stone'] for n in TARGETS):raise ValueError('Old finish material assignment changed')
    ...
    coll['source_guards']=json.dumps({n:source[n] for n in GUARDS},sort_keys=True)
