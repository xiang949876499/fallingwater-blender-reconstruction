...
r=json.loads((q/'guest-dimensions-measured.json').read_text(encoding='utf-8'));e=json.loads((q/'guest-dimensions-measured-projection.json').read_text(encoding='utf-8'))
...
make('west','guest-04-upright.png',[205,127,856,305],3.5,'GUEST HOUSE | WEST ELEVATION: ORIGINAL + ACTUAL MESH','Service enclosure discrepancy: actual 3.392675 m; printed top of stone wall 4.511675 m',lambda v:(577-(v[1]-wy)/esx,243-(v[2]-fz)/esz),f'Mapping: px = 577 - (Y - 37.1) / {esx:.9f}; py = 243 - (Z - {fz:.7f}) / {esz:.9f}.', 'Scope: west/service envelope, upper terrace walls, selected slabs/roofs and connector canopy.', ['Independent scale: horizontal 66 ft 2 3/8 in across 384.5 px; vertical 7 ft 8 5/8 in across 44 px.','Anchor: service SE corner at px 577 (+/-2 px); guest datum at py 243. Scale witnesses +/-1 px.','X-ray projection with no occlusion removal; no object moved to force agreement. Blue bracket: 1.119 m.'])
r['overlays']=meta
(q/'guest-dimensions-measured.json').write_text(json.dumps(r,ensure_ascii=False,indent=2),encoding='utf-8')
