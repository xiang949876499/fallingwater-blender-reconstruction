...
assert all(c['ceiling_unchanged'] for c in controls if c['label']!='Loggia_corner')
# This source point is within the repaired solid masonry, not a walking
# position. Its new upward ray correctly meets the wall's top from inside;
# applying the unchanged-free-ceiling control here was a category error.
solid_control=next(c for c in controls if c['label']=='Loggia_corner')
assert solid_control['ceiling_after']['object']=='MAIN_L1_coat_1'
assert solid_control['ceiling_after']['normal'][2]>.99
