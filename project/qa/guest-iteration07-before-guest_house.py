...
def build(ctx):
    ...
    def subtract_box(obj, center, size, label):
        cutter=box('QA_'+label,center,size,concrete,col,0)
    ...
    for roof in data['roofs']:
        ...
        slab_obj=poly_prism(roof['id'],polygon,rz,rz+.19,concrete,col)
        finish_obj=poly_prism(roof['id']+'_weathering',polygon,rz+.19,rz+.235,roofmat,col)
        for i,opening in enumerate(roof.get('openings_px',[])):
            x0,y0,x1,y1=opening;aa,bb=Vector(p((x0,y0))),Vector(p((x1,y1)))
            for target in (slab_obj,finish_obj):
                subtract_box(target,(*(aa+bb)/2,rz+.12),(abs(bb.x-aa.x),abs(bb.y-aa.y),1),'Roof_true_opening_%02d'%i)
        if roof.get('parapet',False):
            parapet=[p(q) for q in roof.get('parapet_polygon',roof['polygon'])]
            for i in range(len(parapet)):
                piece(roof['id']+f'_parapet_{i}',parapet[i],parapet[(i+1)%len(parapet)],rz+.19,z(roof['level'])+roof.get('parapet_top',roof['height']+.63),.13,concrete)
    ...
    for i in range(len(inner)):
        j=(i+1)%len(inner)
        pool_boundary.append(poly_prism('GUEST_POOL_shell_%02d'%i,[inner[i],inner[j],outer[j],outer[i]],basin,pooltop-.07,concrete,col))
        pool_boundary.append(poly_prism('GUEST_POOL_coping_%02d'%i,[inner[i],inner[j],outer[j],outer[i]],pooltop-.07,pooltop,floor,col))
    ...
    if notch:
        ...
        # The old rounded perimeter also crossed the source stair entrance.
        # Cut both shell and coping above the dry approach datum, retaining the
        # basin walls below ground and the uninterrupted non-entry perimeter.
        x0=a.x-.60;x1=b.x+.03;y0=a.y-width/2-.015;y1=a.y+width/2+.015
        for ob in pool_boundary:
            coords=[v.co for v in ob.data.vertices]
            if max(q.x for q in coords)<x0 or min(q.x for q in coords)>x1 or max(q.y for q in coords)<y0 or min(q.y for q in coords)>y1:continue
            subtract_box(ob,((x0+x1)/2,(y0+y1)/2,gz+1.5), (x1-x0,y1-y0,3.002),'Source_pool_stair_boundary_opening')
            ob['source_entry_cut']='Guest01 dry stair atx696.5..721.5,y425..445; upper boundary genuinely removed'
    ...
    if not any(r.get('openings_px') for r in data['roofs']):
        for i in range(8):
