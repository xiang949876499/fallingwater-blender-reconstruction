...
assert all('course' in x['object'] for x in rays if x['pixel'] in [[100,200],[150,300]])
assert report['all_nontarget_unchanged']
(R/'qa/main-interface10-final-reopen.json').write_text(json.dumps(report,indent=2),encoding='utf-8');print(json.dumps({k:v for k,v in report.items() if k not in ['courses','corrected_manifest']},indent=2))
