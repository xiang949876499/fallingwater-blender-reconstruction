...
# The initial conservative maxScale^2/full-mesh estimate was rejected before
# baking. Use the preserved read-only actual world-triangle refinement instead.
preflight_path=OUT/'preflight-refined.json'
budget=json.loads(preflight_path.read_text(encoding='utf-8'))
assert budget['status']=='READ_ONLY_COMPLETE' and budget['source_sha256']==EXPECTED
assert abs(budget['density32_per_m']-32/max(probe.scale))<1e-6
incremental_allowance=budget['planning_allowance_bytes']
...
report['resource_preflight']={k:v for k,v in budget.items() if k not in ['all_objects','largest_projected_area_objects']}
report['resource_preflight'].update(total_physical_memory_bytes=memory.total,
    available_physical_memory_bytes=memory.available,guard_limit_bytes=budget_limit,
    estimate_source=str(preflight_path),estimate_source_sha256=sha(preflight_path),
    initial_estimate_preserved=str(OUT/'report-initial-preflight.json'))
REPORT.write_text(json.dumps(report,indent=2),encoding='utf-8')
print('DENSITY_PREFLIGHT '+json.dumps(report['resource_preflight']),flush=True)
