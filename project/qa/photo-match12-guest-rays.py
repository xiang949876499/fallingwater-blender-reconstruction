...
eye=Vector(fit['camera']['eye_m']);R=Matrix(fit['camera']['world_to_opencv']);f=fit['camera']['focal_px']
...
for p in fit['points']:
    ...
    px,py=p['pixel'];direction=R.transposed()@Vector(((px-371)/f,(py-512)/f,1))
...
for p in fit['points']:
    ...
    ev.to_mesh_clear()
out={'scene_sha256':fit['scene_sha256'],'camera_source':'photo-match12-guest-fit.json','rows':rows,'support_probes':support,
...
(ROOT/'qa/photo-match12-guest-rays.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
