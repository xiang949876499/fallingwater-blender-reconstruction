...
def study_floor_source_polygon():
 """Physical floor to actual inner faces, distinct from the inset room census.

 Main06 shows an enclosed Study with a southeast chimney return. Measured
 existing wall inner faces define the boundary; 5mm laps enter wall volume.
 The gallery-threshold notch avoids coplanar duplicate finish. No stair or
 exterior floor is added. Wall locations themselves retain C trace evidence.
 """
 world=[(-3.987,16.4153),(-.3098,16.4153),(-.3098,13.9122),
        (-.4716,13.9122),(-.4716,12.5847),(-.3398,12.5847),
        (-.3398,11.7284),(-.6238,11.7284),(-.6238,11.3584),
        (-.9364,11.3584),(-.9364,11.2822),(-2.5152,11.2822),
        (-2.5152,11.5434),(-3.987,11.5434)]
 return [(x/SX+ORIGIN[0],ORIGIN[1]-y/SY) for x,y in world]

def closed_floor_source_polygon(rid):
 """08 bounded physical slabs; C plan traces and actual modeled inner faces.

 The census is not a construction edge. Five-millimetre wall laps close the
 Bath N/Dressing perimeter; bath door connections share edges with thresholds.
 Gallery only gains a door-side notch: no staircase/alcove void is filled.
 """
 world={
  'MAIN_L2_BATH_N':[(.2896,16.2598),(3.4158,16.2598),(3.4158,14.4851),(.2896,14.4851)],
  'MAIN_L2_BATH_G':[(5.053,12.5097),(7.2086,12.5097),(7.2086,10.5888),(5.053,10.5888)],
  'MAIN_L2_BATH_M':[(5.053,10.0671),(7.1562,10.0671),(7.1562,6.8187),(5.053,6.8187)],
  'MAIN_L2_DRESSING':[(-4.0194,16.3953),(-2.358,16.3953),(-2.358,16.3822),
   (-.2224,16.3822),(-.2224,14.337),(.1572,14.337),(.1572,12.744),
   (-.9432,12.744),(-.9432,11.5758),(-4.0194,11.5758)]
 }
 if rid in world:return [(x/SX+ORIGIN[0],ORIGIN[1]-y/SY) for x,y in world[rid]]
 if rid=='MAIN_L3_GALLERY':
  return [(333,278),(338,278),(338,283),(363,283),(363,278),(365,278),
          (365,291),(434,291),(434,304),(331,304),(331,303),(333,303)]
 if rid=='MAIN_L1_SERVANT':
  # Only the west wall-side floor gap: keep the northern bay and its narrow
  # source opening beside the rock edge. No opaque connector across that slot.
  west=-7.5978/SX+327
  return [(183.5,228),(193,226),(202,227),(205.5,214.7),(244.5,214.7),
          (244.5,229.4),(238,229.4),(238,269),(west,269),(west,230)]
 if rid=='MAIN_L2_CLOSET_M':
  # 09: the Master slab/finish already covers y312 onward here. Preserve
  # this north strip and the semantic wardrobe footprint independently.
  return rect(337,308,366,312)
 if rid=='MAIN_L3_BATH':
  # 09 main06: the enclosed north pocket extends east of the old inset
  # census rectangle. Follow the actual stepped inner faces, with 5mm laps;
  # retain the existing south doorway edge and its gallery threshold.
  p=[(.4094,16.1105),(3.2686,16.1105),(3.2686,15.3778),
     (1.834,15.3778),(1.834,13.9122),(.4094,13.9122)]
  return [(x/SX+327,540-y/SY) for x,y in p]
 if rid=='MAIN_L3_ALCOVE':
  # 09: only the demonstrated north corner inside the existing north wall.
  # Keep the lower west edge at x435, beside the actual stair/gallery space.
  north=540-15.1247/SY
  return [(432,north),(468,north),(468,302),(435,302),(435,259),(432,259)]
 return None

def closed_ceiling_source_polygon(rid):
 """08b shared ceilings above enclosed wardrobes; no duplicated soffit faces."""
 if rid=='MAIN_L2_CLOSET_M':return rect(337,308,368,312)
 if rid=='MAIN_L2_CLOSET_G':return rect(484,328,534,342)
 return None

def guest_corridor_ceiling_source_polygon():
 # The Guest room provides y342 onward at x469 onward. Retain the original
 # one-pixel west strip and the actual stair-side corridor outline.
 return [(468,301),(536,301),(536,328),(484,328),(484,342),
         (469,342),(469,344),(468,344)]

def physical_threshold_source_rect(tid, original):
 """Only bridge actual room edges; no coplanar duplicate cork finish."""
 return {
  'guest_bath':(7.2086/SX+327,312,468,333),
  'master_bath':(415,365,5.053/SX+327,384),
  'dressing_bath':(333,540-14.4851/SY,354,272),
  'hall_dressing':(330,271,331,300),
  'dressing_terrace':(241,281,-4.0194/SX+327,296),
  'gallery_bath':(338,278,363,283),
 }.get(tid,original)

def physical_threshold_source_polygon(tid, original):
 if tid=='entry_loggia':
  # Remove Entry and Loggia overlap while retaining the original sloped
  # north-edge connection: Loggia's first edge runs (535,307)..(565,308).
  return [(510,306),(537,306),(537,307+2/30),(535,307),(535,332),(510,332)]
 if tid=='entry_coat':
  # 09: subtract the existing COAT and ENTRY floors from the old threshold.
  # Keep the one-pixel connection at y300..301 and the original side returns.
  return [(492,296),(495,296),(495,300),(512,300),
          (512,303),(510,303),(510,301),(492,301)]
 return rect(*physical_threshold_source_rect(tid,original))

def entry_east_corner_source_polygon():
 """09 main04 hatched L-core: close only the butt-ended outer quadrant.

 Existing .51m wall faces constrain this local infill. Five-millimetre laps
 enter the two existing legs; no surrounding entry/stair floor is added.
 """
 return rect(527-.005/SX,334-.255/SY,527+.255/SX,334+.005/SY)

def core_north_backing_source_polygon():
 """Main05 C trace: solid east/back of the west-facing recess, not its mouth.

 The existing straight x316 relief ran through a real recess at py303..322.
 This stepped backing follows the hatched mass and rejoins the old south core.
 """
 return [(325,303),(332,303),(332,327),(327,327),(327,329),
         (310,329),(310,322),(325,322)]
...
def room_records():
 ...
 for rid,label,lev,p,c,e,kind in ROOM_SPECS:
  ...
  if rid=='MAIN_L3_STUDY':
   out[-1]['physical_floor_source_polygon']=study_floor_source_polygon()
   out[-1]['physical_floor_polygon']=[list(xy(p)) for p in study_floor_source_polygon()]
   out[-1]['notes']+=' The census polygon is inset; physical slab/finish separately continue to measured inner wall faces and window sill, retaining the chimney steps and existing gallery threshold.'
  physical=closed_floor_source_polygon(rid)
  if physical:
   out[-1]['physical_floor_source_polygon']=physical
   out[-1]['physical_floor_polygon']=[list(xy(p)) for p in physical]
   out[-1]['notes']+=' Physical floor revision08 follows actual modeled wall faces or a door-side shared-edge notch. Census polygon retained separately; C source trace precision is unchanged.'
  ceiling=closed_ceiling_source_polygon(rid)
  if ceiling:
   out[-1]['physical_ceiling_source_polygon']=ceiling
   out[-1]['physical_ceiling_polygon']=[list(xy(p)) for p in ceiling]
   out[-1]['notes']+=' Ceiling revision08b shares its boundary with adjoining room/corridor soffits; semantic room bounds remain unchanged.'
...
def export_data(path=None):
 ...
 for t in result['thresholds']:
  physical_rr=physical_threshold_source_rect(t['id'],t['source_rect'])
  if list(physical_rr)!=t['source_rect']:
   t['physical_source_rect']=list(physical_rr)
   t['physical_revision']='08 shared-edge connection; removes coincident room/threshold finish'
  if t['id']=='entry_loggia':
   t['physical_source_polygon']=physical_threshold_source_polygon(t['id'],t['source_rect'])
   t['physical_revision']='08b shared edges remove Entry/Loggia duplicates, including sloped north corner'
  if t['id']=='entry_coat':
   t['physical_source_polygon']=physical_threshold_source_polygon(t['id'],t['source_rect'])
   t['physical_revision']='09 shared edges remove COAT/ENTRY duplicates; original connecting strip and side returns retained'
...
def build(ctx):
 ...
 def stone_courses(name,a,b,z0,z1,t=.45,clip_start=0.):
  ...
  while z<z1-.04:
   ...
   pos=-.36*(row%2);j=0
   while pos<length:
    start=max(0,pos,clip_start); end=min(length,pos+.58+.23*((row+j*3)%4)/3)
    if end>start+.04:
     for side in (-1,1):
 ...
 for r in records:
  ...
  p=r.get('physical_floor_source_polygon',r['source_polygon']); z=r['z'];floor(r['id']+'_slab',p,z,.22,rid=r['id'])
 ...
 mark(prism('MAIN_L1_entry_east_corner_core',entry_east_corner_source_polygon(),.1,2.58,'stone'),None,'wall')
 ...
 # Main07 south/Main08 west elevations independently place the exposed terrace
 # band at Z-.49..+1.08, including its downward structural portion. C +/- .055m
 # graphical reading; main04 XY and terrace floor datum are unchanged.
 chain('MAIN_L1_west_parapet',[(236,450),(236,543),(322,543)],-.49,1.08,.22,'ochre')
 ...
 # Main10 section supports the lower slab edge. Its window-seat upstand is a
 # separate footprint; do not turn the entire south fascia into a guard wall.
 chain('MAIN_L1_south_fascia',[(236,543),(600,543)],-.49,-.015,.18,'ochre')
 ...
 prism('MAIN_L2_core_north_backing',core_north_backing_source_polygon(),2.8448,5.21,'stone')
 # Keep the original lower course positions; remove only the unsupported north
 # extension across the source recess. The new east backing does not fill it.
 stone_courses('MAIN_L2_core',(316,305),(316,381),2.8448,5.22,.62,clip_start=20*SY)
 ...
 # The entire former rect(468,142,474,176) lower landing is already supplied
 # by MAIN_L2_TERRACE_N slab/finish. A duplicate pair caused a black coplanar
 # strip at the first stair; retain the real terrace and all spiral steps.
 ...
 for r in records:
  ...
  cp=r.get('physical_ceiling_source_polygon',r['source_polygon'])
 ...
 for tn,rid,rr,lev,mat,probe in THRESHOLDS:
  physical_p=physical_threshold_source_polygon(tn,rr)
  zz=LEVELS[lev];ob=floor('MAIN_floor_threshold_'+tn,physical_p,zz,.18,mat,rid=rid);ob['component_type']='floor';ob['threshold_id']=tn
  ob=prism('MAIN_floor_threshold_'+tn+'_finish',physical_p,zz,zz+.022,mat,rid=rid);ob['component_type']='floor';ob['threshold_id']=tn
 floor('MAIN_B_STAIR_base_floor',ROOM_SPECS[3][3],-2.15,.18,'stone_floor',rid='MAIN_B_STAIR')
 prism('MAIN_L2_guest_corridor_ceiling',guest_corridor_ceiling_source_polygon(),5.0,5.018,'ceiling',rid='MAIN_L2_HALL')
