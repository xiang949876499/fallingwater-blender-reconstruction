...
R=Path(__file__).resolve().parents[1];SRC=R/'scene/Fallingwater_structure_candidate08b.blend'
assert hashlib.sha256(SRC.read_bytes()).hexdigest()=='77ecfeaa879a49432d727844c299ea618b021ec08015149717bf88f0559ec3cb'
...
groups={'CAM_MAIN_L1_LOGGIA_A':[(270,429),(289,409),(615,359),(638,373),(494,275),(499,293)],'CAM_MAIN_L2_MASTER_A':[(207,478),(418,109)]}
bvhs={};bounds={}
for ob in s.objects:
...
for name,pixels in groups.items():
    ...
    for x,y in pixels:
        ...
        z=.122 if '_LOGGIA_' in name else (5. if y<200 else 2.8668);expected=origin+d*((z-origin.z)/d.z);plane_surfaces=[]
        for nm,bvh in bvhs.items():
...
(R/'qa/structure08b-remaining-review.json').write_text(json.dumps(result,indent=2));print(json.dumps(out,indent=2))
