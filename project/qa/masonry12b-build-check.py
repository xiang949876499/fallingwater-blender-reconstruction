...
try:
 ...
 for name in entry.MODIFIED:
  ...
  allowed_properties={'data','dimensions'} if name.startswith('MAIN_stone_tower_') else {'data'}
  assert all(v==b['properties'][k] for k,v in a['properties'].items() if k not in allowed_properties),name
  if 'dimensions' in allowed_properties:
   old_dims=a['properties']['dimensions'];new_dims=b['properties']['dimensions']
   assert old_dims[:2]==new_dims[:2] and abs(new_dims[2]-old_dims[2]-app['correction_m'])<2e-6,name
