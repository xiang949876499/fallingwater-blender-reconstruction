...
result={'status':'PASS_GEOMETRY_C_HEIGHT_NOT_VISUAL_ACCEPTANCE' if dimension_ok and body_ok and after_stair is None and not any(after_connections) and not new_frame_failures and same else 'REQUIRES_REVIEW',
    ...
    'actual_core_face_clear_width_m':measurement,'nominal_deviation_m':measurement-nominal if measurement is not None else None,'tolerance_m':.020,'dimension_status':'PASS' if dimension_ok else 'FAIL',
...
candidate=ROOT/'scene/guest_stair_wall_candidate08.blend'
...
(ROOT/'qa/guest-stair-wall08-check.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
