...
source=root/'qa/guest-iteration06-module.blend'
bpy.ops.wm.save_as_mainfile(filepath=str(source))
(root/'qa/guest-iteration06-rooms.json').write_text(json.dumps(rooms,ensure_ascii=False,indent=2),encoding='utf-8')
s=(root/'qa/guest-dimensions-measured-extract.py').read_text(encoding='utf-8').replace("root/'scene/Fallingwater_working.blend'","root/'qa/guest-iteration06-module.blend'").replace('guest-dimensions-measured-projection.json','guest-iteration06-projection.json').replace('guest-dimensions-measured.json','guest-iteration06-dimensions.json').replace('saved iteration03','fresh iteration06 module')
exec(compile(s,'guest-iteration06-dimension-regression','exec'),{})
s=(root/'qa/guest-geometry-clearance-check.py').read_text(encoding='utf-8').replace('guest-geometry-clearance-report.json','guest-iteration06-upper-clearance.json')
exec(compile(s,'guest-iteration06-upper-clearance','exec'),{})
...
report={'scope':'Fresh guest-only module. Full iteration06 integrated 61 adjacency and 120 cameras require retest.','scene_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),'doors':doors,'laundry_descent':stair,'pool_dry_stairs':pool,'fireplace_real_cavity':fire,'counts':{'door_pass':sum(r['status']=='PASS' for r in doors),'door_total':len(doors),'laundry_pass':sum(r['status']=='PASS' for r in stair),'pool_pass':sum(r['status']=='PASS' for r in pool),'fireplace_pass':sum(r['status']=='PASS' for r in fire)}}
(root/'qa/guest-iteration06-geometry.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
print('ITERATION06_GEOMETRY',json.dumps(report['counts']))
