"""Read persisted full09 cameras before independently testing their actual poses.
...
SCENE_SHA = freeze['scene_sha256']
SETTINGS_SHA = next(v for k,v in freeze['inputs'].items() if k.replace('\\','/') == 'data/camera-settings-reviewed.json')
scene_path = ROOT / freeze['checkpoint']
active_path = ROOT / 'data/camera-settings-reviewed.json'
frozen_path = ROOT / 'qa/camera09-settings-frozen.json'
...
camera_review.write_json(ROOT / 'qa/camera09-saved-settings-check.json', saved_report)
print('CAMERA09_SAVED ' + json.dumps({'status': saved_report['status'], 'count': len(saved_rows)}), flush=True)
actual_path = ROOT / 'qa/camera09-saved-geometry-input.json'
camera_review.write_json(actual_path, actual_config)
geometry_report = camera_review.verify(scene, rooms, actual_path, ROOT / 'qa/camera09-verification.json')
...
camera_review.write_json(ROOT / 'qa/camera09-verification.json', geometry_report)
summary = {
    ...
    'scope': 'Full integrated09 checked independently. Room geometry excludes vegetation. Legacy 15-ray view diagnostic excludes shift, and is not visual acceptance. Does not validate navigation or full120 framing/exposure/photo quality.',
    'full09_visual_status': 'NOT_REVIEWED_BY_THIS_CHECK',
    'final_visual_quality_accepted': False,
}
camera_review.write_json(ROOT / 'qa/camera09-validation-summary.json', summary)
print('CAMERA09_VALIDATED ' + json.dumps(summary), flush=True)
