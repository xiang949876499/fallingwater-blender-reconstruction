...
def apply(*,enabled=False):
    ...
    # Fit only perimeter intersections to the existing real stone relief and
    # west window's corner mullion. The protected objects are not modified.
    # This removes actual small interpenetrations, not a fictitious interior
    # ceiling slot around an incorrectly inward-opening leaf.
    cutters=['MASTER_DETAIL10_hearth_tall_return',
             'MASTER_DETAIL10_hearth_return_course_01_349',
             'MASTER_DETAIL10_hearth_return_course_01_358',
             'MASTER_DETAIL10_hearth_return_course_02_349',
             'MASTER_DETAIL10_hearth_return_course_02_358',
             'MAIN_L2_master_west_mullion_0']
    for n in cutters:
        mod=ceiling.modifiers.new('CEILING11_fit_actual_boundary','BOOLEAN')
        mod.operation='DIFFERENCE';mod.solver='EXACT';mod.object=s.objects[n]
        with bpy.context.temp_override(object=ceiling,active_object=ceiling):
    ...
    # A half-turn retains the baked square hinge-member orientation. The
    # tempting -156deg transformation for outward78deg rotates that square
    # corner into adjacent fixed glass (attempt01 retained). Outward102deg
    # is an explicitly C pose; direction, sizes and hinge are preserved.
    angle=-math.pi
    ...
    for n in LEAF_NAMES:
        ...
        o['master_ceiling11_swing']='B outward direction; C magnitude102deg; original pivot/size'
    ...
    return {'status':'APPLIED_TOPOLOGY_CANDIDATE','changed':list(CHANGED),'removed':list(REMOVED),'added':[],
            ...
            'outward_leaf_angle_degrees':math.degrees(theta+angle),'perimeter_fit_cutters':cutters,
